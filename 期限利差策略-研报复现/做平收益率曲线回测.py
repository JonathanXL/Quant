...
# 做平策略回测函数
# =============================================
def flatten_strategy_backtest(data, D1, D2):
    """
    做平策略回测函数
    D1: 建仓分位数上界
    D2: 平仓分位数下界
    ...
    for i in range(len(data)):
        ...
        # 建仓条件：分位数>=D1
        if current_position is None and row['quantile'] >= D1:
        ...
        if current_position is not None:
            ...
            # 计算持有期收益率盈利
            long_yield_change = current_position['entry_long_yield'] - current_long_yield # 长期债券收益率变动
            short_yield_change = current_short_yield - current_position['entry_short_yield'] # 短期债券收益率变动
...
# 参数优化：网格搜索函数
# =============================================
def grid_search_optimize(data, strategy_type='flatten'):
    """
    参数优化主函数
    strategy_type: 'flatten'做平策略
    """
    # 生成参数网格
    if strategy_type == 'flatten':
    ...
    # 遍历所有参数组合
    for D1, D2 in tqdm(params_grid, desc='参数搜索进度'):
        # 执行回测
        positions, total_gain = flatten_strategy_backtest(data, D1, D2)  # 调用做平策略回测函数
...
# =============================================
# 做平策略优化（示例）
optimize_results = grid_search_optimize(data, strategy_type='flatten')
...
# 使用最优参数执行回测
# =============================================
best_D1 = best_params['D1']  # 最优建仓分位数
best_D2 = best_params['D2']  # 最优平仓分位数
final_positions, final_gain = flatten_strategy_backtest(data, best_D1, best_D2)  # 执行回测
...
if final_positions:
    ...
    final_positions_df.to_excel('C:/Users/35343/Desktop/flatten_final_positions.xlsx', index=False)